...
class Newsletter extends Controller {
	...
	public function index() {		
		...
		return view('admin/newsletter/vwManageNewsletter', $data);
	...
	public function get(Request $request) {
		...
		if ($request->ajax()) {
			...
			$allColumns = \Schema::getColumnListing('newsletter');
		    ...
		    $searchValue = $search_arr['value']; // Search value
		     ...
		     // Total records
		    $totalRecords = NewsletterModel::select('count(*) as allcount')->count();
		    $totalRecordswithFilter = NewsletterModel::select('count(*) as allcount');
		    ...
		    $totalRecordswithFilter = $totalRecordswithFilter->count();
		     ...
		     // Fetch records
		    $records = NewsletterModel::orderBy('id','desc')->skip($start)->take($rowperpage);
	...
	public function delete(Request $request) {
		if ($request->ajax()) {
	        ...
	        if ($validator->fails()) {
	        	...
	        	$getData = NewsletterModel::where('id', '=', $id)->first();
	        	
	        	if (!empty($getData)) {
	        		
	        		$isDeleted = NewsletterModel::where('id', $id)->delete();

        			if ($isDeleted) {
        				$this->status = array(
							'error' => false,								
							'msg' => 'Newsletter has been deleted successfully.'
						);
        			} else {
        				$this->status = array(
							'error' => true,
	...
	public function bulkDelete(Request $request) {
		if ($request->ajax()) {
	        ...
	        if ($validator->fails()) {
	        	...
	        	$getData = NewsletterModel::whereIn('id', $ids)->get();	        	
	        	
	        	if (!empty($getData)) {

	        		$isDeleted = NewsletterModel::whereIn('id', $ids)->delete();
